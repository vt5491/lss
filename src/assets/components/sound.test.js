...
suite('sound', function () {
  ...
  suite('fadeOut', function() {
    test('function fadeOut exists', function () {
      var el = this.el;
      var soundComp = el.components.sound;
      // debugger;
      assert(typeof el.components.sound.fadeOut === "function");
      assert(soundComp.data.fadeOutTime === 0);
    })
  });

  suite('initPoolState', function () {
    test('initPoolState creates array of objects of correct size', function () {
      var el = this.el;
      var soundComp = el.components.sound;
      // debugger;
      soundComp.initPoolState();
      assert(Array.isArray(soundComp.poolState));
      assert.equal(soundComp.poolState.length, soundComp.data.poolSize);
      assert.equal(typeof soundComp.poolState[0], 'object');
    })
  })
  //vt end
});
...
//vt add
suite('fade sound', function () {
  setup(function (done) {
    console.log(`sound.test.js: now in fade sound setup`);
    var el = this.el = entityFactory();
    el.setAttribute('sound', {
      fadeOutTime: 1000,
    });
  ...
  test('fadeOutTime is set properly', function() {
    var el = this.el;
    var soundComp = el.components.sound;
    // debugger;
    assert(soundComp.data.fadeOutTime === 1000);
  })
